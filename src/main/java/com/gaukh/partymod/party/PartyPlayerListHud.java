...
public class PartyPlayerListHud extends TickingSystem<EntityStore> implements PartyEventListener {
    ...
    private void updateHudVisibility(@Nonnull UUID playerUuid, @Nullable Party party) {
        ...
        boolean shouldShow = shouldShowHud(party);
    ...
    private boolean shouldShowHud(@Nullable Party party) {
        if (party == null) {
    ...
    private void checkForMissingHuds() {
        ...
        for (Party party : partyManager.getAllParties()) {
            for (UUID playerUuid : party.getMemberUuids()) {
                ...
                if (!state.hudVisible && shouldShowHud(party)) {
    ...
    private void updateAllMemberStats(Store<EntityStore> store) {
        for (ViewerHudState viewerState : viewerStates.values()) {
            if (!viewerState.hudVisible) continue;

            try {
                updateMemberStatsForViewer(viewerState, store);
            } catch (Exception e) {
    ...
    private void updateMemberStatsForViewer(@Nonnull ViewerHudState viewerState, Store<EntityStore> store) {
        ...
        if (viewer == null || viewer.getWorld() == null) return;
        ...
        updateSingleMemberStats(viewerRef, viewerMemberState, viewerX, viewerY, viewerZ);
        ...
        for (UUID memberUuid : party.getMembersExcept(viewerUuid)) {
            ...
            updateSingleMemberStats(viewerRef, memberState, viewerX, viewerY, viewerZ);
        }
    ...
    private void updateSingleMemberStats(@Nonnull PlayerRef viewerRef,
        ...
        if (online) {
            ...
            if (memberPlayer != null) {
                // Get health and stamina from EntityStatMap component via PlayerRef
